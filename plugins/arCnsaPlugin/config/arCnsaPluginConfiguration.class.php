...
class arCnsaPluginConfiguration extends sfPluginConfiguration
{
  public static
    $summary = 'Theme plugin, extension of arDominionPlugin.',
    $version = '0.0.1.1-2.1.0';
  ...
  public function contextLoadFactories(sfEvent $event)
  {
    ...
    if ($context->getConfiguration()->isDebug())
    {
      ...
      $context->response->addStylesheet('/plugins/arCnsaPlugin/css/main.less', 'last', array('rel' => 'stylesheet/less', 'type' => 'text/css'));
    }
    else
    {
      $context->response->addStylesheet('/plugins/arCnsaPlugin/css/min.css', 'last', array('media' => 'screen'));
    }
    $context->response->addJavaScript('/plugins/arCnsaPlugin/web/memns', 'last');
  }
